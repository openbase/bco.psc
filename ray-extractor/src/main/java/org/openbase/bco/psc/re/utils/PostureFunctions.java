...
import static rst.tracking.PointingRay3DFloatType.PointingRay3DFloat.PointingType;
...
public class PostureFunctions {
    ...
    public static final Collection<PointingRay3DFloat> getAllRaysForSideAndTypesWithConfidence(TrackedPosture3DFloat posture, 
        ...
        return types.stream()
                .map(type -> getPointingRayWithConfidence(posture, right, pointingProbability, type))
    ...
    public static final boolean checkConfidences(TrackedPosture3DFloat posture, boolean right){
        if(posture.getConfidence(Joints.Head.getValue()) < 1)
            return false;
        if(right){
            return posture.getConfidence(Joints.ShoulderRight.getValue()) == 1 && 
                    posture.getConfidence(Joints.ElbowRight.getValue()) == 1 && 
                    posture.getConfidence(Joints.WristRight.getValue()) == 1 && 
                    posture.getConfidence(Joints.HandRight.getValue()) == 1;
        } else {
            return posture.getConfidence(Joints.ShoulderLeft.getValue()) == 1 && 
                    posture.getConfidence(Joints.ElbowLeft.getValue()) == 1 && 
                    posture.getConfidence(Joints.WristLeft.getValue()) == 1 && 
                    posture.getConfidence(Joints.HandLeft.getValue()) == 1;
        }
    }
